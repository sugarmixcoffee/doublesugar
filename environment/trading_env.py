...
class TradingEnvironment:
    ...
    def __init__(
        ...
        symbol: str = None,
        reward_type: str = "original"  # "original" 또는 "trade_encouraging"
    ):
        """
        ...
        Args:
            ...
            symbol: 주식 심볼
            reward_type: 보상 함수 타입 ("original" 또는 "trade_encouraging")
        """
        # 데이터 확인 및 숫자형 변환
        if data.empty:
            raise ValueError("입력 데이터가 비어있습니다.")
        
        # 문자열/날짜 컬럼 제거하고 숫자형만 유지
        numeric_columns = data.select_dtypes(include=[np.number]).columns
        self.data = data[numeric_columns].astype(np.float32)
        
        # 필요한 컬럼 확인
        required_columns = ['open', 'high', 'low', 'close', 'volume']
        missing_columns = [col for col in required_columns if col not in self.data.columns]
        if missing_columns:
            warnings.warn(f"필수 컬럼이 누락되었습니다: {missing_columns}")
        ...
        self.reward_type = reward_type  # 보상 함수 타입 저장
        ...
        LOGGER.info(f"{self.symbol} 트레이딩 환경 초기화 완료: 데이터 길이 {self.data_length}, 보상 타입: {self.reward_type}")
    ...
    def _execute_trade_action(self, action: float) -> None:
        ...
        # 거래 임계값 설정 (reward_type에 따라 다르게 설정)
        if self.reward_type == "trade_encouraging":
            TRADE_THRESHOLD = 0.05  # 더 낮은 임계값으로 거래 장려
        else:
            TRADE_THRESHOLD = 0.1  # 기존 임계값
        ...
        if action_value > 0:  # 매수
            ...
            # 행동 값에 따라 매수할 주식 수 결정
            if self.reward_type == "trade_encouraging":
                # 전체 범위 사용
                shares_to_buy = min(
                    max_affordable,
                    self.max_trading_units * (action_value / 1.0)
                )
            else:
                # 임계값으로 정규화
                shares_to_buy = min(
                    max_affordable,
                    self.max_trading_units * (action_value / TRADE_THRESHOLD)
                )
            ...
            if self.reward_type == "trade_encouraging":
                ...
                shares_to_sell = min(
                    self.shares_held,
                    self.max_trading_units * (abs(action_value) / 1.0)
                )
            else:
                # 임계값으로 정규화
                shares_to_sell = min(
                    self.shares_held,
                    self.max_trading_units * (abs(action_value) / TRADE_THRESHOLD)
                )
    ...
    def _calculate_reward(self, prev_portfolio_value: float, current_portfolio_value: float) -> float:
        ...
        """
        if self.reward_type == "trade_encouraging":
            return self._calculate_reward_trade_encouraging(prev_portfolio_value, current_portfolio_value)
        else:
            return self._calculate_reward_original(prev_portfolio_value, current_portfolio_value)
    
    def _calculate_reward_original(self, prev_portfolio_value: float, current_portfolio_value: float) -> float:
        """
        기존 보상 계산 함수
        """
        ...
        if position_change_ratio < 0.1:  # 포지션 변화가 작은 경우
            ...
            holding_bonus = min(0.01 * self._no_trade_steps, 0.1)  # 최대 0.1까지 보너스
        ...
        if self._no_trade_steps >= 60:  # 60스텝 이상 거래가 없는 경우
            ...
            time_penalty = -min (0.001 * (steps_over_60+1), 0.2)  # 최대 0.2까지 패널티
    ...
    def _calculate_reward_trade_encouraging(self, prev_portfolio_value: float, current_portfolio_value: float) -> float:
        """
        거래를 장려하는 개선된 보상 계산 함수
        """
        if prev_portfolio_value <= 0:
            return 0
        ...
        # 1. 기본 수익률 보상
        return_rate = (current_portfolio_value - prev_portfolio_value) / prev_portfolio_value
        base_reward = return_rate * 100  # 1% 변화 = 1.0 보상
        ...
        # 거래 활동 보상 (적절한 거래를 장려)
        if 0.1 < position_change_ratio < 0.5:  # 적절한 크기의 거래
            trade_reward = 0.05  # 작은 보상
        elif position_change_ratio >= 0.5:  # 큰 거래
            trade_reward = -0.1  # 과도한 거래는 페널티
        else:  # 거래 없음
            trade_reward = 0
        
        # 3. 홀딩 패널티 (거래하지 않으면 패널티)
        if position_change == 0:
            self._no_trade_steps += 1
            # 거래하지 않는 시간이 길수록 더 큰 패널티
            holding_penalty = -0.01 * min(self._no_trade_steps, 20)  # 최대 -0.2
        else:
            self._no_trade_steps = 0
            holding_penalty = 0
        
        # 4. 포지션 크기에 따른 보상 조정
        position_ratio = self.shares_held / self.max_trading_units if self.max_trading_units > 0 else 0
        if position_ratio > 0.8:  # 포지션이 너무 큼
            position_penalty = -0.05
        elif position_ratio < 0.2 and self.balance > prev_portfolio_value * 0.5:  # 현금을 너무 많이 보유
        ...
        # 5. 수수료 페널티
        current_commission = self.total_commission - self._previous_commission
        commission_penalty = current_commission / prev_portfolio_value * 50 if prev_portfolio_value > 0 else 0
        
        # 최종 보상 계산
        reward = base_reward + trade_reward + holding_penalty + position_penalty - commission_penalty
