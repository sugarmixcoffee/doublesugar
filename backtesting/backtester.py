...
class Backtester:
    ...
    def __init__(
        ...
        transaction_fee_percent: float = 0.0025,
    ...
    def run_backtest(self, verbose: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            # print(f"[DEBUG] reset() 반환값 타입: {type(reset_result)}")
            # print(f"[DEBUG] reset() 반환값: {reset_result}")
            state = reset_result
            done = False
            total_reward = 0
            step_count = 0
            ...
            # 디버깅: 초기 상태 데이터 타입 확인
            if verbose:
                # print(f"[DEBUG] Initial state type: {type(state)}")
            ...
            for step in iterator:
                ...
                # 거래 실행 후 포트폴리오 변화 확인
                # print(f"[DEBUG] step {step}: balance changed from {info.get('balance', 0):.2f} to {info.get('balance', 0):.2f}")
                # print(f"[DEBUG] step {step}: shares_held changed from {info.get('shares_held', 0)} to {info.get('shares_held', 0)}")
                # print(f"[DEBUG] step {step}: portfolio_value changed from {info.get('portfolio_value', 0):.2f} to {info.get('portfolio_value', 0):.2f}")
                
                # # 보상 계산 내역 확인
                # print(f"[DEBUG] step {step}: reward = {reward:.4f}")
                # print(f"[DEBUG] step {step}: prev_portfolio_value = {info.get('portfolio_value', 0):.2f}")
                # print(f"[DEBUG] step {step}: current_portfolio_value = {info.get('portfolio_value', 0):.2f}")
                # print(f"[DEBUG] step {step}: position_change_ratio = {info.get('position_change_ratio', 0):.4f}")
            ...
            if self.logger:
                self.logger.error(f"백테스트 실행 중 오류 발생, Backtester.py: {str(e)}")
            raise
        ...
        try:
            ...
            save_data = {
                "backtest_info": {
                    ...
                    "commission_rate": self.transaction_fee_percent,
                    "test_period": {
    ...
    def run(self, verbose: bool = True) -> dict:
        """
        백테스트 실행 진입점. 내부적으로 run_backtest()를 호출합니다.
        Args:
            verbose: 진행 상황 표시 여부
        Returns:
            백테스트 결과 dict
        ...
        return self.run_backtest(verbose=verbose) 